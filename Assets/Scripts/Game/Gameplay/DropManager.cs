...
public class DropManager : MonoBehaviour
{
    ...
    void Start()
    {
        DataBase data = GetComponent<DataBase>();

        int dropCount = data.InventoryItems.Count;

        for (int i = 0; i < dropCount; i++) // заполнение массива droplist
        {
            droplist[i].id = data.InventoryItems[i].Id;
            droplist[i].name = data.InventoryItems[i].Name;
            droplist[i].img = data.InventoryItems[i].Img;
        }
    }
